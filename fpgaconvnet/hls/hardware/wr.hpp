...
typename weight_t
>
void weights_reloading_1d_horizontal(
    stream_t(weight_t) &in,
    weight_t weights[COARSE_IN*COARSE_GROUP][COARSE_OUT][DIVIDE(FILTERS,COARSE_IN*COARSE_GROUP*COARSE_OUT*KERNEL_SIZE)][1][KERNEL_SIZE]
)
{
...
#pragma HLS STREAM variable=in
    ...
    const unsigned int coarse_out    = COARSE_OUT;
    const unsigned int coarse_group  = COARSE_GROUP;
    const unsigned int filters       = DIVIDE(FILTERS, COARSE_IN*COARSE_GROUP*COARSE_OUT*KERNEL_SIZE);
    const unsigned int kernel_size = KERNEL_SIZE;

    coarse_in_loop: for(unsigned int coarse_in_index=0; coarse_in_index < coarse_in*coarse_group; coarse_in_index++) {
        coarse_out_loop: for(unsigned int coarse_out_index=0; coarse_out_index < coarse_out; coarse_out_index++) {
            filter_loop: for(unsigned int filter_index=0; filter_index < filters; filter_index++) {
	    	kernel_2_loop: for(unsigned int k_index=0; k_index < kernel_size; k_index++) {
		    #pragma HLS unroll
		    #pragma HLS pipeline II=1
		    #pragma HLS dependence variable=weights inter false
		    weights[coarse_in_index][coarse_out_index][filter_index][0][k_index] = in.read();
                }
            }
        }
    }
